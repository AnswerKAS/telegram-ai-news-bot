...
- Bot messages are **HTML** (`parse_mode: "HTML"`) in Russian, with the `SYSTEM_PROMPT` in `bot/summarizer.py`. Keep the response format (2-3 Russian sentences, no intro phrases) intact.
...
- `bot/summarizer.py` — model `z-ai/glm-5.2` on OpenRouter is hardcoded here (`summarizer.py:11`); renders the HTML message (title in `<b>`, "Читать далее →" link, source).
...
- Workflow `.github/workflows/news_bot.yml` runs every 6h on Python 3.12, then commits `data/sent_articles.json` back.

## Reference

`CLAUDE.md` is the maintained project doc — keep it in sync if this file drifts.
